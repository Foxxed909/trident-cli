...
const ANTHROPIC_MODELS = [
  ...
  { name: 'claude-sonnet-4-6    — balanced, $3/$15 per M tok  (rec)', value: 'claude-sonnet-4-6' },
  { name: 'claude-haiku-4-5-20251001 — fast & cheap, $0.25/$1.25',    value: 'claude-haiku-4-5-20251001' },
];
...
export async function runOnboarding(): Promise<void> {
  ...
  const { mode } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        {
          name: chalk.hex(ROSE).bold('yolo') + chalk.hex(SLATE)('     — approve everything automatically (trust the AI fully)'),
          value: 'yolo',
        },
        {
          name: chalk.hex(AMBER).bold('lockdown') + chalk.hex(SLATE)(' — ask before every single action (maximum control)'),
          value: 'lockdown',
        },
      ],
      default: 'review',
  ...
  if (alreadySet) {
    console.log('  ' + chalk.hex(TEAL)(`✓ ${envKey} is already set in your environment — skipping.`));
  } else {
    ...
    console.log('  ' + chalk.hex(SLATE).dim(`  export ${envKey}=your-key-here`));
    console.log('');
    ...
    if (apiKey && apiKey.trim().length > 0) {
      ...
      console.log('');
      console.log('  ' + chalk.hex(TEAL)('✓ Key set for this session.'));
      console.log('  ' + chalk.hex(SLATE).dim(`  To persist it, add this to your shell profile (~/.bashrc, ~/.zshrc, etc.):`));
      console.log('  ' + chalk.hex(SLATE).dim(`  export ${envKey}=${apiKey.trim().slice(0, 12)}...`));
    } else {
      console.log('');
  ...
  for (const [k, v] of rows) {
    console.log('    ' + chalk.hex(SLATE)(k) + '  ' + chalk.white(v));
  }
  console.log('');
